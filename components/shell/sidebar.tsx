      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            />
            <motion.aside
              initial={{x:-300}} animate={{x:0}} exit={{x:-300}} transition={{type:'spring', damping:24}}
    ...
    <div className="flex flex-col w-full h-full glass border-r border-gold-soft px-5 py-7">
        ...
        {NAV.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href} onClick={onItemClick}
              className={cn(
                'group relative flex items-center gap-3 px-3.5 py-3 rounded-xl text-sm transition-all',
                active
                  ? 'text-foreground bg-gradient-to-r from-gold-500/15 to-transparent border border-gold-500/30 shadow-inner-gold'
                  : 'text-muted-foreground hover:text-foreground hover:bg-white/[0.03]'
              )}
            >
              {active && (
                <motion.div layoutId="activeNav"
                  className="absolute left-0 top-1.5 bottom-1.5 w-[3px] rounded-full bg-gold-gradient shadow-gold-glow" />
              )}
      ...
      <div className="mt-6 p-4 rounded-xl glass-gold">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="font-display text-sm">Studio Pro</span>
        </div>
