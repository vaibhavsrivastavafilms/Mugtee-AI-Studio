...
type ReelControlCenterProps = {
  projectId?: string
  audioRef?: RefObject<HTMLAudioElement | null>
  className?: string
}

/** Phase B — dedicated completion experience (does not return to editor). */
export function ReelControlCenter({ projectId, audioRef, className }: ReelControlCenterProps) {
  ...
  }, [pid, state.title, state.hook, state.voiceName, state.style, state.niche, state.language, state.duration])
  ...
  return (
    <div className={cn(v4PanelClass, 'flex flex-col min-h-0 h-full overflow-hidden', className)}>
      <div className="shrink-0 px-4 pt-4 pb-2 border-b border-white/[0.06] text-center">
        <p className="text-[10px] tracking-[0.24em] uppercase text-emerald-200/90">✓ Your Reel Is Ready</p>
        {timing ? (
          <div className="mt-2 flex flex-wrap justify-center gap-x-4 gap-y-1 text-[11px] text-luxe/60">
            {timing.generation ? (
              <span>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-luxe px-3 sm:px-4 py-3 space-y-4">
        ...
        <div className="flex flex-wrap justify-center gap-2 pb-2">
          ...
          {isPublishLayerEnabled() ? (
            ...
            <button
              type="button"
              className={cn(extraActionClass, 'opacity-50 cursor-not-allowed')}
              disabled
              title="Enable NEXT_PUBLIC_ENABLE_PUBLISH_LAYER to open publish workspace"
            >
              <Share2 className="w-3.5 h-3.5" aria-hidden />
              Publish
            </button>
