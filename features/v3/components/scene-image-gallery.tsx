...
import type { V3SceneImageMetadata, V3SceneImageRow, V3SceneRow } from '@/types/v3/production'

type SceneImageGalleryProps = {
  projectId: string
  scenes: V3SceneRow[]
...
export function SceneImageGallery({
  ...
  const latestByScene = useMemo(() => {
    const latest = pickLatestSceneImages(sceneImages)
    return new Map(latest.map((image) => [image.scene_id, image]))
  }, [sceneImages])
  ...
  async function regenerateScene(sceneId: string) {
    ...
    setError(null)
    try {
      const res = await fetch(`/api/v3/projects/${projectId}/images/regenerate`, {
        method: 'POST',
  ...
  return (
    <>
      <section className={cn('rounded-2xl border border-white/[0.08] bg-[#0a0a0a]/80 p-4 sm:p-5', className)}>
        <p className="text-[10px] uppercase tracking-[0.22em] text-white/45">Scene images</p>
        {error ? <p className="mt-2 text-sm text-red-300">{error}</p> : null}
        <ul className="mt-4 space-y-3">
          {Array.from(latestByScene.entries()).map(([sceneId, image]) => {
            const scene = sceneById.get(sceneId)
            const metadata = image.metadata as V3SceneImageMetadata
            const isOpen = openScenes[sceneId] ?? false
            const historyCount = sceneImages.filter(
              (row) => row.scene_id === sceneId && row.status === 'completed'
            ).length

            return (
              <li key={image.id} className="rounded-xl border border-white/[0.06] bg-black/30">
                <button
                  type="button"
                ...
                >
                  ...
                  <span className="font-medium text-white/90">
                    Scene {scene?.number ?? '?'} master frame
                  </span>
                  <span className="ml-auto text-xs text-white/35">{image.provider}</span>
                </button>
                  ...
                  <div className="space-y-4 border-t border-white/[0.06] px-4 py-4">
                    ...
                    <dl className="grid gap-2 text-xs sm:grid-cols-2">
                      {[
                        ...
                        ['Generation time', image.generation_time_ms ? `${image.generation_time_ms}ms` : '—'],
                        ['Prompt version', metadata.promptVersion ? `v${metadata.promptVersion}` : '—'],
                        ['History', `${historyCount} version${historyCount === 1 ? '' : 's'}`],
                        ['Seed', image.seed != null ? String(image.seed) : '—'],
