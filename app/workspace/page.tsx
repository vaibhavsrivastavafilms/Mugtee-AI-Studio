...
// Starter prompts ΓÇö clicking auto-fills the topic textarea (does NOT auto-generate).
// Phase 3N ΓÇö Expanded cinematic starter pool. A fresh slice of 5 is shuffled
...
// Static showcase ΓÇö purely for inspiration/proof. No data, no carousel, no animation.
...
// =====================================================================
// Phase 2F ΓÇö Silent Language Normalization (client-side detection only).
...
// Hinglish markers ΓÇö strictly Hindi-origin Romanized words. NO words that
...
export default function WorkspacePage({ embeddedProjectId }: { embeddedProjectId?: string } = {}) {
  ...
  // Phase 3A ΓÇö Creator Memory + Stability. Quiet trust signals.
  const [lastSavedAt, setLastSavedAt] = useState<number | null>(null)
  const [savedFlash, setSavedFlash] = useState(false)  // Phase 3I ΓÇö transient "Γ£ô Saved" pill.
  const [recovered, setRecovered] = useState(false)  // true if mount hydrated from localStorage
  // V1.10 ΓÇö Payoff "magic moment": bumped on each successful generation/load so the
  ...
  // ΓöÇΓöÇ Telemetry refs (avoid stale closures + unnecessary rerenders) ΓöÇΓöÇ
  ...
  useEffect(() => {
    ...
    fetch('/api/projects/recent').then(r => r.json()).then(d => {
      if (!alive) return
      // Phase 3Q ΓÇö Defensive dedup. The API itself returns unique rows, but
  ...
  // Lifecycle telemetry ΓÇö capture abandonment + unsaved exits via pagehide (more reliable than beforeunload on mobile).
  useEffect(() => {
    if (typeof window === 'undefined') return
    const onLeave = () => {
      const dwellMs = Date.now() - sessionStartTsRef.current
      // Empty session = signed-in, never generated, idled >= 20s ΓåÆ useful "bounced from empty workspace" signal.
      ...
      // Generated but never saved ΓÇö important activation signal.
  ...
  const loadProject = useCallback(async (id: string, opts?: { syncUrl?: boolean; tab?: string }) => {
    ...
    try {
      ...
      // Phase 3A ΓÇö restore active tab if caller asked, else default to hook.
      ...
      // Project loaded successfully ΓåÆ mark this moment as "saved" for the trust signal.
      ...
      // Project age (days) + legacy flag ΓÇö useful for retention + churn analysis.
  ...
  // Phase 3A ΓÇö persist active tab to localStorage so refresh restores it.
  useEffect(() => {
    try { window.localStorage.setItem(TAB_KEY, tab) } catch {}
  }, [tab])

  // Phase 3E ΓÇö react to `?tab=` deep links so sidebar nav (Scripts /
  ...
  // Director Cut upgrade modal — surfaced when arriving via ?upgrade=1 (locked entry points).
  useEffect(() => {
  ...
  // Phase 3A ΓÇö remember the most recent project id so we can recover it on next visit.
  useEffect(() => {
    if (!savedId) return
    try { window.localStorage.setItem(LAST_PROJECT_KEY, savedId) } catch {}
  }, [savedId])

  // Phase 3A ΓÇö quiet debounced autosave. Only fires for already-saved projects so
  ...
  // Phase 3A ΓÇö auto-fade the "Recovered from previous session" pill after a moment.
  useEffect(() => {
    if (!recovered) return
  ...
  // Phase 3A ΓÇö refresh the relative "Saved moments ago" label every 30s. The
  ...
  // Phase 3C ΓÇö Workflow Clarity. Stage derived from existing state only.
  const stage = useMemo<CreativeStage>(() => deriveStage(output, tab), [output, tab])

  // Phase 3N ΓÇö randomized starter slice + cinematic welcome greeting.
  // Phase 3N-hf ΓÇö Hydration fix. Server render MUST be deterministic, so we
  ...
  const generate = async () => {
    ...
    try {
      ...
      // Phase 3I ΓÇö auto-route the creator into Script tab right after the
      // first generation completes. Encourages the natural narrative flow:
      // Script ΓåÆ Storyboard ΓåÆ Frames ΓåÆ Voice.
      setTab('script')
      setRevealNonce(n => n + 1)
  ...
  const saveProject = async (opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      setSavedId(data.id)
      setLastSavedAt(Date.now())  // Phase 3A ΓÇö power the "Saved moments ago" indicator.
      // Phase 3K ΓÇö transient inline "Project safely saved" confirmation (~2.5s).
      setSavedFlash(true)
      ...
      // Phase 3A ΓÇö silent autosaves never bother the creator with toasts. They just
  ...
  // V1.10 (Phase 2A) ΓÇö helper for any feature that needs a persisted project id
  ...
  const newProject = () => {
    ...
    // Phase 3A ΓÇö also clear the persistence pointer so refresh doesn't restore the old project.
    ...
    <div className="flex min-h-screen flex-col bg-noir-radial">
      ...
      <aside className="lg:w-64 lg:shrink-0 border-b lg:border-b-0 lg:border-r border-white/[0.06] bg-black/40 backdrop-blur-xl">
        <div className="p-4 lg:p-5 space-y-5">
          ...
          {/* Phase 3 ΓÇö quiet creator navigation. Compact muted icons. No analytics, no agents.
              Phase 3E ΓÇö every nav target now stays inside the cinematic workspace
          ...
          <nav aria-label="Mugtee navigation" className="space-y-0.5">
            ...
            {/* Phase 3N ΓÇö Storyboards / Voiceovers route to the cross-project
          ...
          {/* Phase 3H ΓÇö Templates + Showcase removed.
        ...
        <div className="space-y-1 mb-3">
          <div className="flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2.5 text-[10.5px] text-luxe/45">
              {/* Phase 3I ΓÇö transient cinematic save confirmation. Appears for
        ...
        <Card className="p-4 md:p-5 bg-black/40 backdrop-blur-xl border-white/[0.06] space-y-4">
          ...
          <Button
            ...
            className={
              ...
              // Phase 3D visual hierarchy ΓÇö gold-gradient ONLY during Idea
          ...
          {/* Phase 3H ΓÇö Creative Journey moved to BELOW the primary CTA so it
      ...
      {/* FOOTER — Mugtee Output stage tabs (Quick Cut pattern) */}
      <footer
        className={cn(
...
// =====================================================================
// PHASE 3B ΓÇö Cinematic Voiceover Layer (UI slice)
...
function VoiceoverPanel({
  ...
  // Restore pill choice across sessions ΓÇö mirrors Mood / Camera locks.
  useEffect(() => {
    try {
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      // Phase 3G ΓÇö auto-save the project before generation so the voiceover
      ...
      if (!res.ok) {
        ...
        // Backend may return narration even when TTS fails ΓÇö preserve it so
  ...
  return (
    <div className="space-y-3">
      {/* Voice preset pills ΓÇö mirrors Mood / Camera lock styling. */}
      ...
      {/* Header ΓÇö title + generate / regenerate action. */}
        ...
        <Button
          ...
          className={
            ...
            (audio
              ...
              // Phase 3C ΓÇö next-step emphasis: bold gold CTA before first render.
              : 'bg-gold-gradient text-black hover:opacity-90 shadow-gold-glow')
          }
      ...
      {/* Empty / hint state ΓÇö only shown before first generation. */}
      ...
      {/* Busy state ΓÇö small placeholder; no flashy audio UI. */}
      {busy && (
        <div className="space-y-2">
          {/* Phase 3K ΓÇö muted status text during voice synthesis. */}
...
// =====================================================================
// EXPORT CONTROLS ΓÇö Copy All / Download TXT / Download MD
...
function formatAsText(out: GenOutput, title?: string): string {
  ...
  if (title?.trim()) {
    ...
    parts.push('ΓÇö'.repeat(Math.min(title.trim().length, 48)))
    parts.push('')
  }
...
function ExportControls({ output, projectTitle, savedId }: { output: GenOutput; projectTitle?: string; savedId?: string | null }) {
  ...
  // Phase 3M ΓÇö notify the new /api/workspace/exports endpoint so the project
  ...
  const copyAll = async () => {
    try {
      ...
      toast.error('Could not copy ΓÇö try the Download buttons instead.')
...
// =====================================================================
// Phase 2H ΓÇö SHOT TIMING ENGINE
...
// Estimates per-shot duration (2ΓÇô5s) and assigns one of four cinematic
...
`
  ...
  // Climactic ΓÇö last or second-last shot, OR keyword signal of a payoff moment.
  ...
  // Emotional Pause ΓÇö intimate close-ups, silences, held looks, breath.
  ...
  // Fast Cut ΓÇö kinetic / aggressive movement, hard cuts, smash transitions.
  ...
  // Slow Burn ΓÇö slow camera moves, push-ins, drifts, dolly, lingering.
  if (/\b(slow|drift|dolly|push[- ]?in|pull[- ]?out|wait|linger|locked[- ]?off)\b/.test(t)) {
    return { index, durationSeconds: 4, tag: 'Slow Burn' }
  }
  // First shot tends to be the hook ΓÇö gentle slow burn feel by default.
  if (index === 0) return { index, durationSeconds: 3, tag: 'Slow Burn' }
  // Wide / establishing ΓÇö short, breath-of-air shots.
...
// =====================================================================
// STORYBOARD FRAMES ΓÇö Phase 2A cinematic still generation
...
// Phase 3A ΓÇö Creator Memory localStorage keys + helpers.
...
// =====================================================================
// Phase 2G ΓÇö CAMERA STYLE LOCK
...
// =====================================================================
// Phase 2E ΓÇö CHARACTER CONSISTENCY (heuristic-only, in-session memory)
...
function buildFramePrompt(
  ...
  // Phase 2H reliability ΓÇö compact baseline. Single line, ~15 words.
  ...
  // Retry path uses baseline only ΓÇö drops mood / continuity / camera to give
  // Gemini the best chance of a clean generation.
  if (opts?.baselineOnly) {
    return `Frame ${index + 1}\n${shot}\n${baseline}`
  }

  // Compact mood suffix ΓÇö short modifier phrase only (no "Visual Mood Lock:" label).
  const moodSuffix = (moodId && MOOD_BY_ID[moodId])
    ? ` Mood: ${MOOD_BY_ID[moodId].suffix}.`
    : ''

  // Compact continuity ΓÇö single descriptor line, no leading sentence.
...
// =====================================================================
// Phase 2D ΓÇö SMART FRAME EXPORT NAMING
...
function StoryboardFrames({
  ...
  // Phase 2E ΓÇö derive recurring characters from the storyboard (in-session memory).
  const characters = useMemo(() => detectCharacters([shots]), [shots])
  const [frames, setFrames] = useState<FrameAsset[]>([])
  const [busy, setBusy] = useState(false)
  const [progress, setProgress] = useState<{ done: number; total: number } | null>(null)
  // Phase 3K ΓÇö generation-confidence signals. Purely local state.
  const [genStartedAt, setGenStartedAt] = useState<number | null>(null)
  const [showSlowHint, setShowSlowHint] = useState(false)
  const [genErrorMsg, setGenErrorMsg] = useState<string>('')

  // Phase 2D ΓÇö Visual Mood Lock. Local state, persisted to localStorage so the
  ...
  // Phase 2G ΓÇö Camera Style Lock. Same pattern as Mood Lock. Persisted locally.
  ...
  // BUG FIX ΓÇö coordinate with generate() via busyRef so the hydration effect
  // does NOT wipe / overwrite frames mid-generation when ensureSaved() flips
  // savedId from null ΓåÆ uuid and re-triggers this effect.
  ...
  useEffect(() => {
    ...
    fetch(`/api/projects/${savedId}/assets?kind=image`)
      ...
      .then(d => {
        ...
        // render in shot order (0,1,2ΓÇª). Legacy rows lacking sequence_index
        ...
        for (const a of raw) {
          ...
          if (typeof seq === 'number' && seq >= 0) {
            if (seen.has(seq)) continue   // newest-first ΓåÆ skip older dupes
            seen.add(seq)
  ...
  // Phase 3K ΓÇö if frame generation runs longer than 8s, surface a calm
  ...
  const generate = async () => {
    if (!canGenerate) return
    // Phase 2H reliability ΓÇö cap initial generation at 3 frames (was 6) to
    ...
    // Phase 3K ΓÇö generation-confidence signals.
    ...
    try {
      ...
      for (let i = 0; i < targetShots.length; i++) {
        ...
        // First attempt ΓÇö full cinematic stack (mood + continuity + camera).
        let fr = await attempt(false)
        // Phase 2H ΓÇö graceful retry on baseline-only prompt if the rich stack failed.
        if (!fr) {
          retryCount++
  ...
  return (
    <div className="mt-4 space-y-3">
      {/* Phase 2E ΓÇö Cinematic Character Continuity. Passive chips, low visual weight. */}
      {characters.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5">
      ...
      {/* Phase 2D ΓÇö Visual Mood Lock pill picker. Subtle, compact, gold-active. */}
      <div className="flex flex-wrap items-center gap-1.5">
      ...
      {/* Phase 2G ΓÇö Camera Style Lock pill picker. Same restrained aesthetic. */}
      <div className="space-y-1">
        <div className="flex flex-wrap items-center gap-1.5">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <Button
          ...
          className={
            'h-8 gap-1.5 text-[11.5px] disabled:opacity-50 ' +
            (frames.length === 0
              // Phase 3C ΓÇö next-step emphasis: bold gold CTA when no frames yet.
              ? 'bg-gold-gradient text-black hover:opacity-90 shadow-gold-glow'
              : 'border-gold-500/30 hover:border-gold-500/60 text-luxe hover:text-gold-200')
          }
      ...
      {/* Phase 3K ΓÇö frame generation progress label + slow-hint + error.
...
if (!shot) {
  ...
  return
                ...
                // Phase 2H reliability ΓÇö same retry pattern as bulk gen.
...
// =====================================================================
// FRAME CARD ΓÇö Phase 2B hover actions
...
function FrameCard({
  ...
  const downloadFrame = async () => {
    // Phase 2D ΓÇö smart export naming: <project>_s{NN}_sh01.png with safe fallback.
    ...
    try {
      ...
      // creator can right-click ΓåÆ save without us touching the URL.
      window.open(frame.url, '_blank', 'noopener,noreferrer')
      toast.message('Opened in a new tab ΓÇö long-press or right-click to save.')
    }
  }
  ...
  return (
    ...
    >
      ...
      {/* Frame number badge ΓÇö always visible */}
      ...
      {/* Hover actions ΓÇö subtle gradient + 3 icon buttons. Soft fade-in, low visual noise. */}
      {!busy && (
...
// =====================================================================
// Phase 3 ΓÇö Cinematic Creator Cockpit helpers
...
// =====================================================================
// Phase 3C ΓÇö Creative Journey breadcrumb
// 5-stage stage indicator: Idea ΓåÆ Script ΓåÆ Storyboard ΓåÆ Frames ΓåÆ Voice.
...
// Floating mascot ΓÇö subtle idle glow, low presence, no motion library required.
...
// =====================================================================
// Phase 3P ΓÇö Creator Trust rotating wisdom. Deterministic SSR (first line),
...
function AIDirectorCard({
  ...
  // Story Feel ΓÇö derived from tone.
  const toneLabel = TONES.find(t => t.value === tone)?.label || 'Cinematic'
  // Cinematic Tone ΓÇö derived from current mood lock.
  const moodLabel = MOOD_BY_ID[mood]?.label || 'Emotional Indie'
  // Pacing Style ΓÇö derived from camera style.
  const cameraLabel = CAMERA_STYLE_BY_ID[cameraStyle]?.label || 'Intimate Handheld'

  // Runtime + dominant pacing tag ΓÇö both derived from the local timing engine.
  ...
  // Frame readiness ΓÇö derived from storyboard existence (no fake numbers).
  ...
  return (
    <div className="rounded-2xl border border-white/[0.05] bg-black/20 backdrop-blur-xl p-3 space-y-2.5">
      ...
      <p className="text-[11px] text-luxe/45 italic leading-snug">
        {(() => {
          // Phase 3C ΓÇö stage-aware caption. Falls back to existing copy when
          // tab info isn't passed in (defensive ΓÇö keeps prior behavior).
      ...
      {/* Phase 3P ΓÇö Creator Trust. Subtle rotating cinematic wisdom below the
