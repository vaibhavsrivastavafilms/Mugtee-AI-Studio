...
/** Pre-render gate ΓÇö storage paths + fresh URLs; fail only when assets genuinely missing. */
export async function verifyRenderExportInputs(params: {
  ...
  if (!allScenesReady) {
    ...
    throw new Error(
      `Cannot export reel ΓÇö storyboard storage missing for scenes: ${missing.join(', ')}`
    )
  }
