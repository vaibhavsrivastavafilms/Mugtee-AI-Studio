/**
 * Read-only V7 image prompt preview — no Pollinations calls, no Pollen spend.
 *
 * Usage:
 *   npm run v7:image-preview -- 9f90d8a3-f7b6-4b6b-9ebe-fa9083c30ebc
 */
...
async function main() {
  ...
  const { previewV7ImagePromptsForSnapshot, formatV7ImagePreviewReport } = await import(
  ...
  const result = previewV7ImagePromptsForSnapshot(snapshot)
  console.log(formatV7ImagePreviewReport(result))
